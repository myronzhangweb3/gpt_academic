            ...
            with gr_L2(scale=1, elem_id="gpt-panel"):
                ...
                with gr.Accordion("基础功能区", open=True, elem_id="basic-panel") as area_basic_fn:
                    with gr.Row():
                        ...
                        for k in groups:
                            variant = groups[k]["Color"] if "Color" in groups[k] else "secondary"
                            groups[k]["Button"] = gr.Button(k, variant=variant, info_str=f'基础功能区: {k}')
                            groups[k]["Button"].style(size="sm")
                            predefined_btns.update({k: groups[k]["Button"]})
                with gr.Accordion("分组展示区", open=True, elem_id="group-panel") as area_crazy_fn:
                    def create_button_group(group_name):
                        # return [gr.Button(group_name+":自定义按钮1", visible=True, variant="secondary", info_str=f'基础功能区: 自定义按钮')]
                        buttons = []
                        for k in functional:
                            if ("Visible" in functional[k]) and (not functional[k]["Visible"]): continue
                            if functional[k]["Group"] == group_name:
                                btn = gr.Button(k, visible=True, variant="secondary", info_str=f'基础功能区: 自定义按钮')
                                btn.style(size="sm")
                                functional[k]["Button"] = btn
                                predefined_btns_group.update({f"group_{k}": f"{group_name}"})
                                buttons.append(btn)
                        return buttons
